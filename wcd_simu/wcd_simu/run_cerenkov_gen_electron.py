...
for i in range(nmu):
    ...
    mus.append(ELECTRON(P, zen, azim, 1000) )
    
np.save('output/parent_electrons_zen-60.npy', mus)

# mus=np.load('output/parent_electrons_zen-0.npy', allow_pickle=True)

for i,thismu in enumerate(mus):
    f=open('log.txt', 'a+')     
    tak=time.perf_counter()
    thismu.go_Cerenkov(H=H, R=180, dl=2e-4)
    photon_l=thismu.photons
    np.save('output/el_zen-60_%i.npy'%i, photon_l)
    thismu.photons=[]
    tik=time.perf_counter()
    f.write("Finished electron %i took %.2f s.\n"%(i, tik-tak))
    f.close()