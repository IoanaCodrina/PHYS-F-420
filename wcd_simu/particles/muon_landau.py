...
class MUON_L(PARTICLE):
    ...
    def dE_dx_land(self, dx):
        ...
        I=7.97e-5 #MeV
        x=dx*rho
        xi=K/2*z**2*Z_A*(x/self.beta**2)
        return xi \
            *( np.log(2*self.me*self.beta**2*self.gamma**2/I)\
                +np.log(xi/I) + 0.2
                         -self.beta**2 ), xi
        # returns most probable E loss in MeV
    ...
    def _range(self, dx=1e-4):
        ...
        rr=0
        while(self.kE>0.1):
            S_mpv, xi = self.dE_dx_land(dx)
            S=float(landau.sample(x_mpv=S_mpv, xi=xi, n_samples=1))
            self.upd_E(self.kE-S)
            e_loss+=S
            rr+=dx
